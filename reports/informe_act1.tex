...
\textbf{Q9 --- las edades no se leyeron de un documento.} El índice de Whipple cuantifica el \textit{age heaping}: la acumulación artificial de edades terminadas en 0 o 5 que ocurre cuando la edad se estima en lugar de verificarse. El valor obtenido, \textbf{163.3}, corresponde a la categoría \textbf{``Tosca''} de la escala de Naciones Unidas e indica un \textbf{63\% más de personas en edades redondas} de las esperables. \texttt{Age} arrastra por tanto un error de $\pm$2--3 años y no admite franjas etarias estrechas. \textit{(El índice y su escala están diseñados para datos censales, no para una cohorte hospitalaria como esta; el valor 163.3 se verificó de forma independiente, pero la etiqueta ``Tosca'' hereda un supuesto de dominio no comprobado. Ver \texttt{data\_dictionary.md}.)}
...
\textit{Nota de transparencia: esta tabla incluye 2 valores de \texttt{DB} (9.0 y 7.0) ya identificados como errores de laboratorio (Q8). Excluirlos cambia la media de \texttt{DB} en $-$1.5\% y su IQR en $-$6.8\%; los números oficiales no se modifican, mismo criterio que con los duplicados.}
...
Contrastados los 84 valores atípicos contra los límites de lo biológicamente posible, \textbf{cero quedan fuera}. \textit{(\texttt{TP}, \texttt{ALB}, \texttt{A/G Ratio}: estimación del equipo, sin fuente publicada; techo de \texttt{Sgot} (7446~U/L): Chang et al. 2007.)} El caso extremo lo confirma: el paciente con \texttt{Sgot} = 4929 es un hombre de 66 años con \texttt{TB} = 11.3 (nueve veces el techo normal), \texttt{Alkphos} = 1110, \texttt{Sgpt} = 1250 y \texttt{ALB} = 2.4. \textbf{Los tres ejes hepáticos alterados a la vez y de forma coherente entre sí} --- no es un error de tecleo, sino un paciente en fallo hepático agudo grave.
...
\subsection{Una brecha observada, y un mecanismo candidato}
...
El dataset presenta una brecha en la tasa de diagnóstico positivo: \textbf{73.47\% en hombres frente al 64.79\% en mujeres}, \textbf{8.7 puntos porcentuales}. Con n=142 mujeres, el test exacto de Fisher da \textbf{p = 0.055} (IC 95\%: $-$0.2 a +17.6~pp, cruza el cero) --- \textbf{en el límite de la significancia convencional}. El aporte de este trabajo es \textbf{medir un mecanismo concreto} que, si la brecha es real, contribuiría a producirla.
...
\textbf{Caveat necesario:} parte de la asimetría es aritmética, ya que la ventana femenina (19--40] es más ancha que la masculina (30--40]. Lo que \textbf{no} es aritmético es que \textbf{70 de las 142 mujeres tengan efectivamente su ALT dentro de esa franja}, frente a 69 de 441 hombres en la suya: una ventana ancha solo atrapa gente si hay gente dentro. Ajustando por ese ancho, la densidad femenina en su franja (0.0235/U$\cdot$L) es \textbf{exactamente un 50\% mayor} que la masculina en la suya (0.0156/U$\cdot$L): no se explica solo por el ancho.
...
\item Chang, S.-W. et al. (2007). \textit{Study on Analytical and Clinically Reportable Ranges.} Korean J Clin Lab Sci, 39(1), 31--36.
\end{itemize}
...
\texttt{notebooks/02\_eda.ipynb} & Sensibilidad del umbral de ALT por sexo y confusión de \texttt{Alkphos} con la edad \\
\texttt{notebooks/02c\_eda\_clustering.ipynb} & Clustering por edad y sexo; por qué no se pueden derivar umbrales de esta cohorte \\
...
\texttt{docs/CHANGELOG\_iteraciones.md} & Los cuatro \textit{loops} CRISP-DM, con fecha, disparador y decisión tomada \\
