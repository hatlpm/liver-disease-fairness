...
| `../PRD_liver_disease_act2_fases_4-9.md` | **Actividad 2** — Fases 4–9 (modelado y evaluación), tareas T1–T8 del enunciado 2 | ✅ **Completada y entregada** (tag `act2-entregada`). Fases 4–9 y E2 cerradas y mezcladas a `main`. **Solo queda la Fase P** (producción/MLOps), que es valor de portafolio y no tiene fecha de entrega |
...
### 🔴 Cómo ejecutar un notebook desde la línea de comandos (corregido 2026-08-16, Fase E2)

**No uses `jupyter nbconvert`.** Este archivo lo recomendó hasta la Fase E2 y
**está roto en esta máquina**, por dos causas independientes que hay que
conocer porque fallan de formas distintas:

1. **Los *shims* `.exe` del venv están bloqueados a nivel de sistema.**
   `jupyter.exe` y `jupyter-nbconvert.exe` devuelven **"Acceso denegado"**
   tanto desde Bash como desde PowerShell. `python.exe` sí ejecuta, y
   `python -m nbconvert` funciona sin problema. Falla ruidosamente, así que
   se detecta enseguida.
2. **El `kernel.json` del venv lanza `argv[0] = "python"` a secas** — sin
   ruta. En un PATH normal eso resuelve al **Python del sistema**
   (`AppData\Local\Programs\Python\Python313`), que no tiene `pandas`,
   `sklearn` ni `imblearn`. Este falla **en silencio hasta el primer
   import**, y el error resultante no señala la causa: parece un problema de
   dependencias, no de intérprete.

Invocación correcta, que resuelve las dos en la misma shell:

```powershell
$env:PATH = "$env:LOCALAPPDATA\venvs\liver-disease-fairness\Scripts;$env:PATH"
& "$env:LOCALAPPDATA\venvs\liver-disease-fairness\Scripts\python.exe" -m nbconvert `
    --to notebook --execute --inplace notebooks/act2_anexo.ipynb
```

⚠️ Durante la ejecución aparecen `KeyError` de
`joblib/externals/loky/backend/resource_tracker.py`. Son **ruido de limpieza
de procesos** (`n_jobs=-1`), no errores del notebook: el resultado real es el
`exit code` y la línea `[NbConvertApp] Writing N bytes`. Para comprobar de
verdad que el notebook está limpio, buscar salidas de tipo `error` en el
`.ipynb` resultante — que es lo que hace
`tests/test_fase_e2_informe.py::test_anexo_sin_errores`.
